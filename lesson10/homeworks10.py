...
def top_3_symbols(text):
    ...
    # here checking if the input is actually a string
    def allowed_input(text):
        if not isinstance(text, str):
            raise TypeError("The input type must be 'str' only")

    allowed_input(text)

    # here checking if the string is not empty
    def input_not_empty(text):
        if len(text) == 0:
            raise ValueError("The string cannot be empty")

    input_not_empty(text)
...
def combined_dict(dict1, dict2):
    for k in dict1.keys():
        if type(dict1[k]) != int:
            raise ValueError(f"the dict1 value '{dict1[k]}' at key '{k}' must be 'int'")
    for l in dict2.keys():
        if type(dict2[l]) != int:
            raise ValueError(f"the dict2 value '{dict2[l]}'at key '{l}' must be 'int'")
...
def fixed_length_list(list):
    ...
    if elt_length_count <1:
        raise ValueError("list elements should have different sizes")
